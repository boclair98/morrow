...
- 추천 회원이 0명인 계정에서 브랜드 빈 상태·조건 넓히기·프로필 다듬기 CTA 확인
- 사진이 없는 계정에서 가상 프로필이나 가상 성공 수치가 노출되지 않는지 확인
- 360px·390px 모바일에서 하단 내비게이션과 필터가 콘텐츠를 가리지 않는지 확인
...
- 프로필 완료 회원도 대표 사진·가능 시간·본인 확인 상태를 별도 신뢰 신호로
  안내하고, 사용자가 프로필 화면에서 바로 보완할 수 있게 합니다.
- 추천 결과가 없을 때 `모두 확인했다`고 단정하지 않습니다. 조건에 따른 결과 없음,
  지역의 신규 회원 준비 중, 네트워크 오류를 서로 다른 상태로 표시합니다.
- 발견 화면의 사진·이름·매치 수치는 실제 승인 데이터만 사용합니다. 빈 상태를
  채우기 위한 시드 회원·후기·성공률 삽입은 금지합니다.
- 가입자 유입은 전국 동시 광고보다 지역·대학 단위 초대 캠페인으로 시작하고,
  각 캠페인의 가입 완료율·첫 관심·첫 대화율을 익명 집계합니다.
