...
class MediaStorage(private val properties: MorrowProperties) {
    ...
    fun put(key: String, content: ByteArray, contentType: String) {
        if (!key.startsWith("profile/") && !key.startsWith("chat/") && !key.startsWith("story/")) throw IllegalArgumentException("unsupported media key")
    ...
    fun fetch(key: String): ByteArray {
        if (!configured || (!key.startsWith("profile/") && !key.startsWith("chat/") && !key.startsWith("story/"))) throw ApiException(404, "사진을 찾지 못했어요")
    ...
    fun delete(key: String?) {
        if (!configured || key.isNullOrBlank() || (!key.startsWith("profile/") && !key.startsWith("chat/") && !key.startsWith("story/"))) return
        runCatching {
