...
프로덕션 URL: <https://morrow.coders.kr> · 현재 상태: **READY** (최신 고도화 릴리스)

## 최신 고도화 릴리스

이번 릴리스는 추천 회원이 적거나 사진이 아직 없는 초기 사용자도 서비스가
미완성처럼 보이지 않도록 발견 화면을 정리하고, 첫 매치까지의 다음 행동을 더
분명하게 만드는 데 초점을 맞췄습니다.

- 추천 0명 상태를 실제 회원처럼 꾸미지 않고, 사진 등록·시간 기반 추천·안전 기준을
  안내하는 브랜드 빈 상태로 개선했습니다.
- 지역·시간·취향 조건을 적용했는데 결과가 없을 때 `조건 넓히기`를 바로 제공해
  탐색을 중단하지 않도록 했습니다.
- 사진, 소개, 관심사, 가능 시간, 본인 확인 상태를 `PROFILE BOOST` 카드로
  보여주고 프로필 화면으로 이어지는 다음 행동을 추가했습니다.
- 회원 사진이 존재할 때만 추천 사진을 노출하며, 사진이 없으면 가상 회원·가상
  성공률·가상 후기를 만들지 않습니다.
- 모바일 하단 내비게이션, 안전 안내, 매치·알림 진입을 유지하고 주요 터치 영역을
  44px 이상으로 검증했습니다.

이 상태는 실제 회원을 모집할 수 있는 공개 베타 기반입니다. 10만 명 운영을
주장하기 전에는 두 개의 실제 계정으로 OAuth → 사진 검수 → 상호 관심 → 채팅 →
약속 → 안전 피드백을 통과시키고, 익명 합성 데이터 부하 시험과 백업 복구 시험을
완료해야 합니다.
